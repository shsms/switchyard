...
| **Lisp-driven consumer-load + cloud-cover profiles** | `(set-meter-power id W)` and `(set-solar-sunlight id PCT)` setters + `(now-seconds)` / `(window-elapsed N)` clock helpers + `(csv-load)` / `(csv-lookup)` CSV interpolation, used together inside `(every)` callbacks to drive arbitrary load shapes and PV cloud-cover schedules (function-defined or CSV-recorded). Microsim's `:per-phase-power 'symbol` and `min-available-power` indirections covered specific cases; switchyard's split is more flexible (the curve can come from any Lisp expression or interpolated file). |
